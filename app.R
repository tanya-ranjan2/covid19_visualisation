...
appLongName = "COVID-19 & its Spectrum Data Visualization Platform"
lastUpdate = Sys.Date()
  ...
  argonDash::argonDashPage(
    ...
    header = argonDash::argonDashHeader(
      ...
      argonRow(
        ...
        argonColumn(
          width = 4,
          h6(HTML(paste0("TEAM 8 ")), style = 'color:white;
                                  text-align: right;
                                  font-size:15px;
                                  margin-bottom: 0em'),
          # h6(HTML(paste0("-20BDA32")), style = 'color:white;text-align: right;font-size:15px;')
        )
        # fixedPanel(
        #   div(
